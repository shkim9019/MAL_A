<%--
  ...
  *   수정일                   수정자                      수정내용
  *  -------    --------    ---------------------------
  *  2020. 2. 26. 오후 12:19:06   sist         최초 생성
  *
  * author 쌍용교육센터 E반
  * since 2009.01.06
  *
...
<body>
		...
		<tr>
			<td colspan="2" align="right">
				<div>
					...
					<input  type="button"  value="취소요청" id="update_canc_btn" />
				</div>
			</td>
		</tr>
	</table>
	...
	<form action="/MAL_A/payment/payment.do" name="searchFrm" id="searchFrm" method="get">
		...
		<input type="text" name="loginId" id="loginId" value="park01" />
		<table>
			<thead>
				<tr>
					<th>체크</th>
			...
			%>
			<tr>
				<td><input type="radio" value="<%=vo.getPno() %>" name="radioPno"/></td>
				<td><%=vo.getPno() %></td>
				<td><%=vo.getSno() %></td>
				<td><%=vo.getRno() %></td>
				<td><%=vo.getStartDt() %></td>
	...
	<script type="text/javascript">
		//취소요청
		$("#update_canc_btn").on("click",function(){
			...
			$.ajax({
			    type:"POST",
			    url:"/MAL_A/payment/payment.do",
			    dataType:"html", 
			    data:{"work_div":"do_updateCanc",
			    	  "modId":$("#loginId").val(),
		...
		function doRetrieve(){
			...
			frm.search_div.value = "1";
			frm.search_word.value = "park01";
