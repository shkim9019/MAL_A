 ...
 *
 */
@WebServlet(description = "좋아요", urlPatterns = { "/roxandrea/heart.do" })
public class HeartCont extends HttpServlet implements ContHandler {
	...
	public void serviceHandler(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		switch(workDiv) {//거래분기
			...
			case "doRetrieveHeart":// 다건조회
				LOG.debug("2====================");
				LOG.debug("2=workDiv=" + workDiv);
				LOG.debug("2====================");
				this.doRetrieveHeart(req, res);
				break;
			default:
				LOG.debug("==================");
	...
	public void doRetrieveHeart(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inHeartVO.setRegId(SesId);
		LOG.debug("inHeartVO=" + inHeartVO);
		inSearchVO.setPageSize(Integer.parseInt(pageSize));
		inSearchVO.setPageNum(Integer.parseInt(pageNum));

		List<StoreVO> outListStoreVO = (List<StoreVO>) heartService.doRetrieveHeart(inHeartVO, inSearchVO);

		LOG.debug("------------------------");
		for (StoreVO StoreVO : outListStoreVO) {
			LOG.debug(StoreVO);
		}
		LOG.debug("------------------------");
		...
		int totalCnt = 0;
		// 총글수
		if (null != outListStoreVO && outListStoreVO.size() > 0) {
			StoreVO totalVO = outListStoreVO.get(0);
			totalCnt = totalVO.getTotal();
		}

		req.setAttribute("listStoreVO", outListStoreVO);
		req.setAttribute("paramVO", inSearchVO);// 파람
		req.setAttribute("totalCnt", totalCnt);// 리뷰총글수

		RequestDispatcher dispatcher = req.getRequestDispatcher("/roxandrea/member_heart_list.jsp");
	...
	public void doDel(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		HttpSession httpSession = req.getSession();
		MemberVO vo = (MemberVO) httpSession.getAttribute("user"); // 세션값 loginID 의 doselectOne
		String regId = vo.getMid(); // selectOne 값에서 Mid를 추출
		LOG.debug("regId: "+regId);
		...
		//서비스 실행
		int flag = this.heartService.doDelete(inHeartVO);
		LOG.debug("4=============================");
	...
	public void doInsert(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		int flag = this.heartService.doInsert(inHeartVO);
